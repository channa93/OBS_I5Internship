...
class Mongo_db {
    ...
    private $slice = 0;
        ...
        *
        * @usage : $this->mongo_db->aggregate_group(array('_id' => array('UserID' => '$UserID')))->aggregate(collection);
        */
        function aggregate_group($group){
            if (empty($group) && !is_array($group)){
                show_error("Operation must be an array to perform aggregate.", 500);
            }
            $this->aggregates[] = array('$group' => $group);
            return ($this);
        }
        /**
        * --------------------------------------------------------------------------------
        ...
        * --------------------------------------------------------------------------------
        * @author DEN Sopharat <sopharat.den@borama.consulting>
        * @copyright (c) 2016-01-07, Borama
        *
        * Perform aggregation on mongodb collection
        * 
        *Sort the documents based on the parameters passed. To set values to descending order,
        *       you must pass values of either -1 (DESC), else they will be
        *       set to 1 (ASC).
        * 
        * @usage : $this->mongo_db->aggregate_group(array('size' => array('$sum' => 1)))->aggregate_sort(array('size' => 1))->aggregate(collection);
        */
        function aggregate_sort($sort){
            if (empty($sort) && !is_array($sort)){
                show_error("Operation must be an array to perform aggregate.", 500);
            }
            $this->aggregates[] = array('$sort' => $sort);
            return ($this);
        }
        /**
        * --------------------------------------------------------------------------------
        * Aggregation Project Operation
        * --------------------------------------------------------------------------------
        *
        * Perform aggregation on mongodb collection
        *
        * @usage : $this->mongo_db->aggregate_project(array('Name'=> 1, '_id'=> 0))->aggregate(collection);
        */
        function aggregate_project($project){
            if (empty($project) && !is_array($project)){
                show_error("Operation must be an array to perform aggregate.", 500);
            }
            $this->aggregates[] = array('$project' => $project);
            return ($this);
        }
        /**
        * --------------------------------------------------------------------------------
        * Aggregation Unwind Operation
        * --------------------------------------------------------------------------------
        *
        * Deconstructs an array field from the input documents to output a document for each element. 
        * Each output document is the input document with the value of the array field replaced by the element
        *
        * @usage : $this->mongo_db->aggregate_unwind('foo')->aggregate(collection);
        */
        function aggregate_unwind($field_name){
            if (!$field_name){
                show_error("Operation must be a string to perform aggregate.", 500);
            }
            $this->aggregates[] = array('$unwind' => '$'.$field_name);
            return ($this);
        }
        /**
        * --------------------------------------------------------------------------------
        ...
        *                    spherical: true
        *                 }
        * @usage : $this->mongo_db->limit($limit+$offset)->offset($offset)->aggregate_geoNear($geoNear_query)->aggregate(collection);
        * 
        * referent: http://docs.mongodb.org/manual/reference/operator/aggregation/geoNear/
        */
        function aggregate_geoNear($geoNear_query){
            if (!$geoNear_query){
                show_error("Operation must be a string to perform aggregate.", 500);
            }
            $this->aggregates[] = array('$geoNear' => $geoNear_query);
            return ($this);
        }
        /**
        * --------------------------------------------------------------------------------
        ...
        * Perform aggregation on mongodb collection
        *
        * @usage : $this->mongo_db->where(array('UserID'=> 123))->aggregate(collection);
        */
        function aggregate($collection){
            ...
            try{
               if(!empty($this->wheres)){
                    array_unshift($this->aggregates,array('$match'=> $this->wheres));
                }
                           if(!empty($this->sorts)){
               ...
               $documents = $this->db->{$collection}->aggregate($this->aggregates);
                $this->_clear();
        ...
        /**
        *       --------------------------------------------------------------------------------
        *       // aggregate $project concat field
        *       --------------------------------------------------------------------------------
        *       //#sopharat 29-12-2014
        *       concat field and select from where
        *
        *       @usage:$this->mongo_db->where($where)->aggregate_concat('foobar',$value,$arr_concat);
        * 
        *       $concat : array concat field
        *       $value : value to where in concat field 
        * 
        *       ex: $this->mongo_db->where()
        *                          ->aggregate_concat('foobar',$value,array('$CountryCode','$PhoneNumber'));
        */
        public function aggregate_concat($collection,$value,$concat){
            $match = array();
            if(!empty($this->wheres)){
                $match[] = array('$match'=>$this->wheres);
            }
            $project = array(
                            array(
                                '$project'=>array(
            ...
            $aggregate = array_merge($match,$project);
            $Result = $this->db->{$collection}->aggregate($aggregate);   
            $this->_clear();  
        ...
        *
        *       @usage : $this->mongo_db->where_match_element('foo', array('foo_sub' =>'123'))->get('foobar');
        */
        public function where_match_element($field,$condtion){
           $this->_where_init($field);
            $this->wheres[$field]['$elemMatch'] = $condtion;
            return ($this);
        }
        /**
        *       --------------------------------------------------------------------------------
        ...
        */
        ...
        public function slice($field = "",$slice_offset = 0,$slice_limit = 999999){
                if((int)$slice_limit == 0){
                        $slice_limit=999999;
                }
            $this->slice = array($field => array('$slice' => array($slice_offset,$slice_limit)));
            return $this;
        }
...
//                      show_error("Insert of data into MongoDB failed: {$e->getMessage()}", 500);
    ...
    */
    public function batch_insert($collection = "", $insert = array())
    {
            ...
            try
            {
                    $this->db->{$collection}->batchInsert($insert, array("w" =>1));
                    if (isset($insert['_id']))
                    {
                            return ($insert['_id']);
                    }
        ...
        */
        ...
        public function update_all($collection = "", $data = array(), $options = array())
        {
                ...
                try
                {
                        $options = array_merge($options, array("w" =>1, 'multiple' => TRUE));
                        $this->db->{$collection}->update($this->wheres, $this->updates, $options);
                        $this->_clear();
                        return (TRUE);
                }
        ...
        *       --------------------------------------------------------------------------------
        *       @author DEN Sopharat <sopharat.den@borama.consulting>
        *       @copyright (c) 2016-01-06, Borama Media
        * 
        *       Ensure an index of the keys in a collection with optional parameters. To set values to descending order,
        *       you must pass values of either -1, FALSE, 'desc', or 'DESC', else they will be
        *       set to 1 (ASC).
        *
        *       @usage : $this->mongo_db->create_index($collection, array('first_name' => 'ASC', 'last_name' => -1), array('unique' => TRUE));
        */
...
//             return (object) MongoDBRef::create($collection, $id, $db);
        ...
        */
        ...
        private function connect()
        {
                ...
                if(!empty($this->mongo_replica_set) && !empty($this->readPreference)){
                        $options['replicaSet'] = $this->mongo_replica_set;
        ...
        */
        ...
        private function connection_string() 
        {
                ...
                $this->mongo_replica_set = trim($this->CI->config->item('mongo_replica_set'));
                $this->readPreference = trim($this->CI->config->item('readPreference'));
